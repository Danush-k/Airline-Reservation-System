...
async function loadAirports() {
  try {
    ...
    if (data.success) {
      ...
      if (selO && selD) {
        selO.innerHTML = '<option value="">Select Origin</option>';
        selD.innerHTML = '<option value="">Select Destination</option>';
        data.data.forEach(a => {
          selO.add(new Option(`${a._id} — ${a.city}`, a._id));
          selD.add(new Option(`${a._id} — ${a.city}`, a._id));
        });
      }
      
      // Admin datalist
      const dl = document.getElementById('dl-airports');
      if (dl) {
        dl.innerHTML = data.data.map(a => `<option value="${a._id}">${a.name} (${a.city})</option>`).join('');
      }
    }
  } catch (e) { console.error('Failed to load airports:', e); }
}

async function loadAircraftForAdmin() {
  try {
    const res = await fetch(`${API}/api/admin/aircraft`);
    const data = await res.json();
    if (data.success) {
      const dl = document.getElementById('dl-aircrafts');
      if (dl) {
        dl.innerHTML = data.data.map(a => `<option value="${a._id}">${a.model} (${a.registration_number})</option>`).join('');
      }
    }
...
function renderOccupancyChart(data) {
  ...
  container.innerHTML = `
    <div class="bar-chart">
      ${data.map((f, i) => {
        const occ = Math.round(f.economy_occupancy || 0);
        const busOcc = Math.round(f.business_occupancy || 0);
        return `
          <div class="bar-group" style="gap:4px">
            ...
            <div style="display:flex;gap:3px;width:100%;align-items:flex-end;height:120px">
              <div class="bar gradient-2" style="height:${Math.max(occ * 1.2, 8)}px;flex:1"></div>
              <div class="bar gradient-1" style="height:${Math.max(busOcc * 1.2, 8)}px;flex:1"></div>
            </div>
...
// ===== ADMIN DATA ENTRY HANDLERS =====
document.getElementById('form-airport')?.addEventListener('submit', async (e) => {
  e.preventDefault();
  const form = e.target;
  ...
  try {
    ...
    if (result.success) {
      toast('Airport added successfully!', 'success');
      form.reset();
      loadAirports(); // refresh origin/dest dropdowns
    } else throw new Error(result.error);
  } catch (err) {
    toast('Error: ' + err.message, 'error');
  }
  btn.disabled = false;
  btn.textContent = 'Insert Airport';
});
...
document.getElementById('form-aircraft')?.addEventListener('submit', async (e) => {
  ...
  const data = {
    ...
    model: fd.get('model'),
    total_seats: parseInt(fd.get('total_seats')),
    seat_config: {
      economy: parseInt(fd.get('econ_seats')),
      business: parseInt(fd.get('biz_seats')),
      first_class: 0
    },
    status: "active"
  };
  ...
  try {
    ...
    const result = await res.json();
    if (result.success) {
      toast('Aircraft added successfully!', 'success');
      form.reset();
      loadAircraftForAdmin();
    } else throw new Error(result.error);
  } catch (err) {
...
document.getElementById('form-flight')?.addEventListener('submit', async (e) => {
  ...
  const data = {
    ...
    aircraft_ref: fd.get('aircraft_ref'),
    origin_ref: fd.get('origin_ref'),
    destination_ref: fd.get('destination_ref'),
    ...
    seat_inventory: {
      ...
      business: { total: 30, available: 30 },
  ...
  try {
    ...
    if (result.success) {
      ...
      form.reset();
      loadAdminStats(); // refresh charts
    } else throw new Error(result.error);
  } catch (err) {
    toast('Error: ' + err.message, 'error');
  }
  btn.disabled = false;
  btn.textContent = 'Insert Flight';
});
